...
/**
 * Обертка для детектора, полученного от драйвера
 */
public class Detector {
    ...
    /**
     * Asynchronously стартует новый поток для уведомления новых слушателей о новом изображении
     */
 ...
 * разница между этими двумя режимами заключается в механизме получения изображения . <br>
 * <br>
 * В режиме блокировки, когда пользователь вызывает метод { @link #getImage()}, устройство запрашивает новый
 * буфер изображения и пользователь должен ждать, пока он станет доступным . <br>
 * <br>
 * В неблокирующем режиме в фоновом режиме работает специальный поток, который постоянно
 * получать новые изображения и кэшировать их внутри для дальнейшего использования. Этот кешированный экземпляр возвращается
 * каждый раз, когда пользователь запрашивает новое изображение. Из-за этого его можно использовать, когда время очень
 * важно, потому что всем пользователям, запрашивающим новое изображение, не нужно ждать ответа устройства. От
 * при использовании данного режима пользователь должен учитывать тот факт, что в некоторых случаях при двух последовательных
 * запросы на получение нового изображения выполняются чаще, чем их может обслужить детектор, то же самое
 * Экземпляр изображения будет возвращен. Пользователь должен использовать метод { @link #isImageNew()}, чтобы различать
 * если возвращаемое изображение не совпадает с предыдущим. <br>
 ...
 * метод. Если включен неблокирующий режим и не указан DelayCalculator,
 * Будет использоваться DefaultDelayCalculator.
 *
 ...
 */
        ...
        if (open.compareAndSet(false, true)) {
            ...
            while (dli.hasNext()) {
                ...
                try {
                    ...
                    LOG.error(String.format("Notify detector open, exception when calling listener %s", l.getClass()), e);
                }
            }
    ...
    /**
     * Возврат базового устройства детестора. В зависимости от драйвера, используемого для обнаружения устройств, это
     * метод может возвращать экземпляры другого класса. По умолчанию { @link DetectorDefaultDevice}
     * возвращается, когда внешний драйвер не используется.
     *
    ...
    /**
     * Полностью освободить устройство захвата. После этой операции детектор больше не может использоваться и
    ...
    /**
     * Изображение трансформируется с помощью преобразователя изображения. Если преобразователь изображения не был установлен, этот метод
    ...
    /**
     * Вернуть список поддерживаемых размеров представления. Он может различаться в зависимости от источника данных детектора.
     *
    ...
    /**
     * Установить пользовательское разрешение. Если вы используете этот метод, убедитесь, что ваш детектор
     * может поддерживать это конкретное разрешение.
     *
     * @param sizes массив пользовательских разрешений, которые будут поддерживаться веб-камерой.
     */
    ...
    /**
     * Установить новый размер просмотра. Новый размер должен точно совпадать с размером по умолчанию или точно
     * такой же, как один из пользовательских.
     *
     * @param size новый устанавливаемый размер представления
...
/**
 * Захватить изображение с детектора и вернуть его. Возвращает объект изображения или ноль, если детектор закрыт
 * или уже отпущен JVM . <br>
 * <br>
 ...
 * можно статически настроить для переключения всех детекторов в режим автоматического открытия. В этом режиме детектор будет
 * автоматически открывается, когда вы пытаетесь получить изображение с закрытого детектора. Пожалуйста, обратите внимание на некоторые
 * побочные эффекты! В случае многопоточных приложений нет гарантии, что один поток
 * не будет пытаться открыть детектор, даже если он был закрыт вручную в другом потоке.
 *
 * @return Захваченное изображение или ноль, если детектор закрыт или удалена JVM
 */
...
/**
 * Получить RAW (сырое) изображение ByteBuffer. Он всегда будет возвращать буфер размером 3 x 1 байт на каждый пиксель, где
 * Компоненты RGB включены (0, 1, 2) и цветовое пространство sRGB . <br>
 * <br>
 * <b>ВАЖНО!</b><br>
 * Некоторые драйверы могут возвращать непосредственно ByteBuffer, поэтому нет гарантии, что базовые байты
 * не будет освобожден при следующей операции чтения образа. Поэтому, чтобы избежать потенциальных ошибок, вы
 * должен преобразовать этот ByteBuffer в массив байтов, прежде чем вы получите следующее изображение.
 *
 ...
 */
    public ByteBuffer getImageBytes() {
        ...
        // некоторые устройства могут поддерживать прямые буферы изображений, и для тех, которые вызывают
        // задача процессора, а для тех, которые не поддерживают прямое изображение
        // буферы, просто конвертируем изображение в байтовый массив RGB
...
/**
 * Получить RAW изображение ByteBuffer. Он всегда будет возвращать буфер размером 3 x 1 байт на каждый пиксель, где
 * Компоненты RGB включены (0, 1, 2) и цветовое пространство sRGB . <br>
 * < br >
 * < b >ВАЖНО!</b>< br >
 * Некоторые драйверы могут возвращать непосредственно ByteBuffer, поэтому нет гарантии, что базовые байты
 ...
 */
    ...
    /**
     * Если базовое устройство реализует настраиваемый интерфейс, передаются указанные параметры
     * к нему. Может вызываться до метода open или позже, в зависимости от устройства.
        *
    ...
    /**
     * Получить список детекторов для использования. Этот метод будет ожидать предопределенный интервал времени для обнаружения детектора.
    ...
    public static List<Detector> getDetectors() throws DetectorException {
        ...
        // ждем около трехсот миллиардов лет, пока это произойдет
    ...
    /**
     * Получить список детекторов для использования. Этот метод будет ждать заданный интервал времени для устройств , чтобы они были обнаружены.
    ...
    /**
     * Получить список устройств для использования. Этот метод будет ждать заданный интервал времени для устройств, чтобы их обнаружить.
          *
    ...
    /**
     * Получить имя устройства. Имя устройства зависит от значения, возвращаемого
     * основной источник данных, поэтому в некоторых случаях это может быть удобочитаемое значение, а иногда может
     * быть каким-то странным числом.
     *
     * @return Name
     */
     ...
     * @return True, если слушатель был добавлен, false, если он уже был там
    ...
    /**
     * Возврат драйвера детектора. При необходимости выполните поиск . <br>
     * <br>
    ...
    /**
     * Установить новый драйвер для использования устройством . <br>
     * <br>
     * <b>Этот метод не является потокобезопасным!</b>
     *
    ...
    /**
     * Установить новый класс драйвера для использования устойством. Класс, указанный в аргументе, должен расширять
     * Интерфейс { @link DetectorDriver} и должен иметь общедоступный конструктор по умолчанию, поэтому экземпляр может быть
     * создан отражением . <br>
     * <br>
     * <b>Этот метод не является потокобезопасным!</b>
     *
     ...
     * <b>ВНИМАНИЕ!!!</b><br>
     * <br >
     * Это экспериментальная функция, которая будет использоваться в основном на этапе разработки. После того, как вы установите дескриптор
     * сигнал TERM в значение true и получение устройств захвата, API захвата веб-камеры будет прослушивать TERM
     * сигнал и попытаться закрыть все устройства после его получения. <b>Эту функцию можно
     * нестабильно на некоторых системах!</b>
     *
    ...
    /**
     * Переключить все детекторы в режим автоматического открытия. В этом режиме каждый детектор будет автоматически открыт
     * всякий раз, когда пользователь попытается получить изображение из экземпляра, который еще не был открыт. Пожалуйста, не забудьте
     * о некоторых побочных эффектах! В случае многопоточных приложений гарантии нет
     * что один поток не будет пытаться открыть веб-камеру, даже если она была закрыта вручную в разных
     * потоков.
     *
    ...
    /**
     * Включен ли режим автоматического открытия. Режим автоматического открытия будет автоматически открывать детектор всякий раз, когда пользователь
     * попытается получить изображение из экземпляра, который еще не был открыт. Пожалуйста, имейте в виду некоторую сторону
     * последствия! В случае многопоточных приложений нет гарантии, что один поток будет
     * не пытайтесь открыть веб-камеру, даже если она была закрыта вручную в другом потоке.
     *
     ...
     *
     * @param l прослушиватель, который нужно удалить
     ...
     *
     * Служба обнаружения @return или нуль, если она еще не создана
     */
    ...
    /**
     * Везвращает детектор с заданным именем или null, если устройство с заданным именем не найдено. Пожалуйста
     * обратите внимание, что конкретное имя детектора может зависеть от порядка его подключения к USB-порту (например,
