...
/**
 * Этот класс используется как глобальная (системная) блокировка, не позволяющая другим процессам использовать один и тот же класс.
 * камера, пока она открыта. Всякий раз, когда детектор открыт, в фоновом режиме выполняется поток, который
 * обновляет блокировку раз в 2 секунды. Блокировка снимается всякий раз, когда детектор закрывается или
 * полностью утилизирован. Блокировка будет оставаться не менее 2 секунд в случае, если JVM не была
 * корректно завершается (из-за SIGSEGV, SIGTERM и т. д.).
 *
 */
public class DetectorLock {
    ...
    /**
     * И детектор,который мы будем запирать.
     */
    ...
    private void write(long value) {
        ...
        if (tmp.renameTo(lock)) {
            ...
            // просто выпрыгнуть из метода, если он успешен, или попытаться переписать
            // содержимое, использующее потоки, если это не удается
            ...
            // перезаписываем содержимое временного файла для блокировки, попробуйте не более 5 раз
    ...
    /**
     * Полностью отключить запорный механизм. После вызова этого метода блокировка не будет
     * любое влияние на время работы детектора.
     */
    public void disable() {
    ...
    /**
     * Проверьте, заблокирован ли детектор.
     *
